%% increase the power in phase III
close all
clear all
...
%         G=chol(Cg)*complex_randn(M*N,1);
        ...
        G=G.';
        a=ones(1,N);
        Y2=Hd_w(:,1)*a+G*diag(hrw(:,1))*phi+delta.*complex_randn(M,N);
...
%             hat_Hg(:,m0)=Cgm*Hmea'/(Hmea*Cgm*Hmea'+delta^2*eye(N))*bar_Rn(:,m0);
        ...
        for k0=2:K
            ...
            for n0=1:tau
                active=(1:M).'+(n0-1)*M;
                Y_tmp=Hd_w(:,k0)+G(:,active)*hrw(active,k0)+delta.*complex_randn(M,1)./sqrt(K-1);
...
%         LS_G=hat_Hg.';
        ...
        for k0=2:K
            ...
            for n0=1:tau
                ...
                hatHu_k(active)=Cgm*Hmea'/(Hmea*Cgm*Hmea'+delta^2*eye(M)/K+var_Hd(:,:,k0))*Y_tmp;
...
grid on
save('baseline_improve.mat','NMSE_D','NMSE_I_LS','NMSE_I_MAP','snr_w');
