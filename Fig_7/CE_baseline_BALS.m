...
%                 hat_Hg(:,m0)=Cgm*Hmea'/(Hmea*Cgm*Hmea'+delta^2*eye(N)+var_Hd(m0,m0,1)*ones(N,N))*bar_Rn(:,m0);
        ...
        Bar_Y2=Bar_Y2.';
        S=phi_t.';
        f0 = BALS_obj(R,Hg,Hu,phi_t);
        while(1)
            Hu_old=Hu;Hg_old=Hg;
            tmp1=kr(S,Hu.').';
            Hg=Bar_Y1*tmp1'/(tmp1*tmp1');
            tmp2=kr(S,Hg);
            Hu=(tmp2'*tmp2)\tmp2'*Bar_Y2;
            gap1=Hu_old-Hu;
            gap2=Hg_old-Hg;
            f1 = BALS_obj(R,Hg,Hu,phi_t);
            if abs(log10(abs(f1-f0)/abs(f0)))>1.5
                break
            end
            f0=f1;
%             gap=sum(abs(gap1(:)).^2)+sum(abs(gap2(:)).^2);
%             if gap<1e-1
%                 break
%             end
        end
...
grid on
save('base_BALS_bound.mat','NMSE_I_MAP','snr_w');
